...
def test(fnToTest: Callable[[], Any]):
    ...
    def test_decorator():
        print(
            f"🧪 Unit {fnToTest.__name__} from {inspect.getfile(fnToTest)}:{inspect.getsourcelines(fnToTest)[1]} ... ",
            end="",
        )
        error = None
...
def tests(classToTest):
    ...
    def tests_decorator():
        ...
        print(
            f"⏳ Starting {classToTest.__name__} from {inspect.getfile(classToTest)}:{inspect.getsourcelines(classToTest)[1]}"
        )
        nErrors, nTests = 0, 0
        start_time = time.perf_counter()
        for member in inspect.getmembers(classToTest):
            ...
            if unitTestMethodName.startswith("_"):
                continue
            if unitTestMethodName == mainTestMethodName:
                continue
            nTests += 1
        ...
        if nErrors == 0:
            ...
            print("❌", end="")
        print(
            f" Passed {nTests-nErrors}/{nTests} test(s) of {classToTest.__name__} after {total_time:.4f} secs\n"
        )
