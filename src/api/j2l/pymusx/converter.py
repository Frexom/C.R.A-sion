...
def toneToFreq(tone: int or str):
    ...
    if type(tone) == str:
        if len(tone) > 3:
            anx.warning(
                "⚠️ Incorrect tone. 3 letters max using anglosaxon notation (i.e. A4, D#5, Gb7)"
            )
            return None
        ...
        if tone < 0 or tone > 8000:
            anx.warning(
                "⚠️ Incorrect frequency. Must be between 110Hz and 8000Hz or in index from 0 to 100!"
            )
            return None
        if tone < 100:
            toneIndex = tone
        ...
        anx.warning(
            "⚠️ Incorrect tone height. Must be between 110Hz and 8000Hz or in index from 0 to 100 or a str using anglosaxon notation"
        )
        return None
    if toneIndex != None:
