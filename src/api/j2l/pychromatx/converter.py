...
def colorFromPercent(percent: float):
    ...
    if percent <= LED_RED_THRESHOLD:
        ...
        g = int(
            255.0
            * (percent - LED_RED_THRESHOLD)
            / (LED_YELLOW_THRESHOLD - LED_RED_THRESHOLD)
        )
    elif percent <= LED_GREEN_THRESHOLD:
        r = int(
            255.0
            - 255.0
            * (percent - LED_YELLOW_THRESHOLD)
            / (LED_GREEN_THRESHOLD - LED_YELLOW_THRESHOLD)
        )
        g = 255
    elif percent <= LED_CYAN_THRESHOLD:
        b = int(
            255.0
            * (percent - LED_GREEN_THRESHOLD)
            / (LED_CYAN_THRESHOLD - LED_GREEN_THRESHOLD)
        )
        g = 255
        ...
        g = int(
            255.0
            - 255.0
            * (percent - LED_CYAN_THRESHOLD)
            / (LED_BLUE_THRESHOLD - LED_CYAN_THRESHOLD)
        )
    elif percent <= LED_MAGENTA_THRESHOLD:
        b = 255
        r = int(
            255.0
            * (percent - LED_BLUE_THRESHOLD)
            / (LED_MAGENTA_THRESHOLD - LED_BLUE_THRESHOLD)
        )
    else:
        r = 255
        b = int(
            255.0
            - 255.0 * (percent - LED_MAGENTA_THRESHOLD) / (1.0 - LED_MAGENTA_THRESHOLD)
        )
    return (r, g, b)
