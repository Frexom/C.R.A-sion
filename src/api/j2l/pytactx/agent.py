...
class IAgentFr:
    ...
    def tirer(
        self, gachette: bool = True, trajectoire: Callable[[int], int] or None = None
    ) -> None:
        """
    ...
    def changerJoueur(
        self, agentId: str, nomAttribut: str, valeurAttribut: Any
    ) -> None:
        """
    ...
    def sabonner(
        self, evenement: str, callback: Callable[[Any, str, Any, Any], None]
    ) -> None:
        """
...
class IAgent:
    ...
    def fire(
        self, enable: bool = True, firepath: Callable[[int], int] or None = None
    ) -> None:
        """
    ...
    def addEventListener(
        self, attributeName: str, callback: Callable[[Any, str, Any, Any], None]
    ) -> None:
        """
...
def exploreSourcesDirs(
    rootpath,
    subdirs=[],
    takeOnlyExts=[".py", ".json"],
    dontTakeExts=[],
    dontTakeFolders=["venv", "j2l"],
    takeHidden=False,
    recursive=True,
    explored={},
):
...
class Agent(IAgent):
    def __init__(
        ...
        if welcomePrint:
            ...
            print(
                "To join next robots fights ⚔️, Follow the link 👉 https://jusdeliens.com"
            )
        if sourcesdir == None:
        ...
        self.__onAttributeChangeCallbacks: dict[
            str, Callable[[Agent, str, Any, Any], None]
        ] = {}
        for attribute in self.__playerKeyToAttribute.values():
            self.__onAttributeChangeCallbacks[attribute[0]] = []
        self.robot: rbx.IRobot = rbx.OvaClientMqtt(
            robotId,
            arena,
            username,
            password,
        ...
        self.robot.addEventListener(
            rbx.RobotEvent.robotConnected, self._onRobotConnected
        )
    ...
    def addEventListener(
        ...
        if attributeName not in self.__onAttributeChangeCallbacks:
            ...
            anx.warning(
                "⚠️ Can only add event on "
                + str(self.__onAttributeChangeCallbacks.keys())
            )
            return
    ...
    def _onRobotConnected(
        self, eventSrc: Any, eventName: str, playerState: dict[str, Any]
    ) -> None:
        Timer(2, self.__afterRobotConnected).start()
        self.robot.setLedFade(64, 128, 255, 700, 4)
        self.robot.playMelody(
            [
                ("E4", 30),
                ("G4", 30),
                ("C5", 30),
    ...
    def _onPlayerChanged(
        ...
        for playerKey, playerValue in playerState.items():
            ...
            anx.debug(
                ...
                + attributeName
                + " changed from "
                + str(attributeValueBefore)
                + " to "
                + str(self.__dict__[attributeName])
            )
            ...
            if attributeClassCallback != None:
                attributeClassCallback(
                    self, attributeValueBefore, self.__dict__[attributeName]
                )
            for callback in onChangeCallbacks:
                callback(
                    ...
                    self.__dict__[attributeName],
                )

    def _onArenaChanged(
        self, eventSrc: Any, eventName: str, arenaState: dict[str, Any]
    ) -> None:
    ...
    def __run__(filepath: str = "settings.json", agentInfos=None):
        ...
        agentConstructor = getattr(
            getattr(
                __import__(
                    agentInfos["factoryDir"], fromlist=[agentInfos["factoryFile"]]
                ),
        ...
        anx.info(
            "⏳ Agent "
            + str(agentInfos["playerId"])
            + " starting on "
            + str(agentInfos["brokerAddress"])
            + " with username "
            + str(agentInfos["username"])
        )
...
class AgentFr(IAgentFr):
    def __init__(
        self,
        nom: str or None = None,
        arene: str or None = None,
        username: str or None = None,
        password: str or None = None,
        url: str or None = None,
        port: int = 1883,
        fluxImage: str or None = "img.jpeg",
        autoconnect: bool = True,
        proxy: bool = True,
        verbosite: int = 3,
        robotId: str or None = "_",
        ...
        if welcomePrint:
            ...
            print(
                "Pour participer aux prochains affrontements ⚔️, rendez-vous sur 👉 https://jusdeliens.com"
            )
        if sourcesdir == None:
            callerstack = inspect.stack()[1]
            sourcesdir = os.path.dirname(os.path.abspath(callerstack[1]))
        ...
        self.__mapEnToFr = {
            ...
            "distance": "distance",
            "color": "couleur",
            "infoPlayer": "infoJoueur",
            "range": "voisins",
            "score": "score",
            "rank": "classement",
            "nFire": "nTirs",
            "nHitFire": "nTirsRecus",
            "nCollision": "nCollisions",
            "nMove": "nDeplacements",
            "nKill": "nTirs",
            "nDeath": "nMorts",
            "nExe": "nExecutions",
            ...
            "map": "carte",
            "infoArena": "chatArena",
            "isGamePaused": "jeuEnPause",
            "gridColumns": "tailleGrilleColonnes",
            "gridRows": "tailleGrilleLignes",
        }
        self.__agent = Agent(
            nom,
            arene,
    ...
    def afficher(self):
        ...
        print(
            "🧭  Pose : " + str(self.x) + "," + str(self.y) + "," + str(self.orientation)
        )
        print("📏 Distance: " + str(self.distance))
...
class AgentFrCibleAleatoire(AgentFr):
    ...
    def actualiser(self, activerSleep: bool = True):
        if self.__cible == None or (
            self.x == self.__cible[0] and self.y == self.__cible[1]
        ):
            self.__cible = (
                randint(0, self.tailleGrilleColonnes - 1),
                randint(0, self.tailleGrilleLignes - 1),
            )
