...
class EventObservable:
    ...
    def addEventListener(
        self, eventName: str, callback: Callable[[Any, str, Any], None]
    ) -> None:
        if eventName not in self.__onEventCallbacks:
            anx.warning("⚠️ Cannot add event listener for event " + eventName)
            anx.warning(
                "⚠️ Can only add event on " + str(self.__onEventCallbacks.keys())
            )
            return
    ...
    def notify(self, eventName: str, value: Any or None = None) -> None:
        if eventName not in self.__onEventCallbacks:
            ...
            anx.warning(
                "⚠️ Can only notify event on " + str(self.__onEventCallbacks.keys())
            )
            return
...
class RobotStateParser:
    ...
    def getBatteryVoltage(self) -> int:
        if (
            "battery" not in self.__robotSensorsState
            or "voltage" not in self.__robotSensorsState["battery"]
        ):
            return 0
    ...
    def getBatteryLevel(self, voltage=None) -> int:
        ...
        if voltage > DefaultClientSettings.batteryMax:
            return 100
        elif voltage < DefaultClientSettings.batteryMin:
            return 0
        else:
            return int(
                100
                * (voltage - DefaultClientSettings.batteryMin)
                / (DefaultClientSettings.batteryMax - DefaultClientSettings.batteryMin)
            )

    def getFrontLuminosity(self) -> int:
        if (
            "photoFront" not in self.__robotSensorsState
            or "lum" not in self.__robotSensorsState["photoFront"]
        ):
            return 0
    ...
    def getBackLuminosity(self) -> int:
        if (
            "photoBack" not in self.__robotSensorsState
            or "lum" not in self.__robotSensorsState["photoBack"]
        ):
            return 0
...
class RobotRequestBuilder:
    ...
    def toURI(self, url):
        ...
        if "led" in req:
            ...
            if (
                req["led"]["animation"] != "custom"
                and req["led"]["animation"] in self.__fromLedAnimationToLedA
            ):
                params.append(
                    "leda=" + self.__fromLedAnimationToLedA[req["led"]["animation"]]
                )
    ...
    def setMotorSpeed(
        self, leftPower: int, rightPower: int, durationInMsecs: int = 1000
    ):
        if leftPower < -100 or rightPower < -100 or leftPower > 100 or rightPower > 100:
            ...
            return
        if (
            type(durationInMsecs) != int
            or durationInMsecs < 0
            or durationInMsecs > 10000
        ):
            anx.warning(
                "⚠️ Incorrect motor speed duration. Should be a positive integer value in ms lesser than 10000 !"
            )
            return
        self.__robotActuatorsRequest["motor"] = [
            [leftPower, rightPower, durationInMsecs]
        ]
    ...
    def setMotorAnimation(self, moves: list[tuple[int, int, int]]):
        for move in moves:
            if len(move) != 3:
                anx.warning(
                    "⚠️ Incorrect move in motor animation. Should be a list of tuples of 3 params : speedMotorLeft, speedMotorRight, durationInMs !"
                )
                return
            for i in range(2):
                if type(move[i]) != int or move[i] < -100 or move[i] > 100:
                    anx.warning(
                        "⚠️ Incorrect move in motor animation. Move speed should be an integer value between -100 (backward) and 100 (forward) !"
                    )
                    return
            if type(move[2]) != int or move[2] < 0 or move[2] > 10000:
                anx.warning(
                    "⚠️ Incorrect move in motor animation. Move duration should be a positive integer value in ms lesser than 10000 !"
                )
                return
    ...
    def setLedTwinkle(
        self, r: int, g: int, b: int, periodInMsecs: int, repeat: int = 0
    ):
        if r < 0 or g < 0 or b < 0 or r > 255 or g > 255 or b > 255:
            anx.warning("⚠️ LED RGB should be between 0 and 255!")
            return
        ...
        if repeat < 0 or repeat > 65535:
            anx.warning(
                "⚠️ LED twinkle repeat should be between 0 (means forever) and 65535 ms!"
            )
            return
        self.__robotActuatorsRequest["led"] = {
            "animation": "twinkle",
    ...
    def setLedFade(self, r: int, g: int, b: int, periodInMsecs: int, repeat: int = 0):
        ...
        if repeat < 0 or repeat > 65535:
            anx.warning(
                "⚠️ LED fade repeat should be between 0 (means forever) and 65535 ms!"
            )
            return
        self.__robotActuatorsRequest["led"] = {
            "animation": "fade",
    ...
    def setLedHue(self, periodInMsecs: int, repeat: int = 0):
        ...
        if repeat < 0 or repeat > 65535:
            anx.warning(
                "⚠️ LED HUE repeat should be between 0 (means forever) and 65535 ms!"
            )
            return
        self.__robotActuatorsRequest["led"] = {
            "animation": "hue",
    ...
    def setLedAnimation(self, colors: list[tuple[int, int, int, int]], repeat: int = 0):
        if repeat < 0 or repeat > 65535:
            anx.warning(
                "⚠️ LED animation repeat should be between 0 (means forever) and 65535 ms!"
            )
            return
        for color in colors:
            if len(color) != 4:
                anx.warning(
                    "⚠️ Incorrect color in led animation. Should be a list of tuples of 4 params : r,g,b, duration !"
                )
                return
            for i in range(3):
                if type(color[i]) != int or color[i] < 0 or color[i] > 255:
                    anx.warning(
                        "⚠️ Incorrect color in led animation. Color rgb should be a positive integer value between 0 (dark) and 255 (bright) !"
                    )
                    return
            if type(color[3]) != int or color[3] < 0 or color[3] > 10000:
                anx.warning(
                    "⚠️ Incorrect color in led animation. Color duration should be a positive integer value in ms lesser than 10000 !"
                )
                return
        self.__robotActuatorsRequest["led"] = {
            "animation": "custom",
    ...
    def playMelody(self, tones: list[tuple[int or str, int]]):
        ...
        for tone in tones:
            if len(tone) != 2:
                anx.warning(
                    "⚠️ Incorrect tone in melody. Should be a list of tuples of 2 params : frequency, duration !"
                )
                return
            toneHeight = tone[0]
            toneDuration = tone[1]
            if type(toneDuration) != int or toneDuration < 0 or toneDuration > 10000:
                anx.warning(
                    "⚠️ Incorrect duration in melody. Should be a positive integer value in ms lesser than 10000 !"
                )
                return
...
class RobotPrinter:
    ...
    def print(self):
        ...
        print(
            "📸 Camera img "
            + str(self.__robot.getImageWidth())
            + "x"
            + str(self.__robot.getImageHeight())
            + " shot after "
            + str(self.__robot.getImageTimestamp())
            + " ms"
        )
        print("")
...
class CameraReader:
    ...
    def onChunkImageReceived(self, data: bytes):
        ...
        if chunkOfs == 0:
            ...
            anx.debug(
                "⚠️ Rx image corrupted. Expected "
                + str(self.__bufImgOffset)
                + "/"
                + str(self.__bufImgExpectedLength)
                + " instead of rx "
                + str(chunkOfs)
                + "/"
                + str(imgLen)
            ...
            with self.__bufImgMutex:
                if len(self.__bufImgComplete) > 0:
                    if self.__dealImgAsChunk:
                        self.__camImg = Image.open(
                            io.BytesIO(b"".join(self.__bufImgComplete))
                        )
                    else:
                        self.__camImg = Image.open(io.BytesIO(self.__bufImgComplete))
                    self.__bufImgComplete = []
                    self.__prevImgRx = (
                        datetime.now() - self.__startTime
                    ).total_seconds() * 1000
                    if self.__camImgOutputPath != None:
                        try:
                            self.__camImg.save(self.__camImgOutputPath)
                            anx.debug(
                                "📸 Save camera image in "
                                + str(os.getcwd())
                                + "\\"
                                + str(self.__camImgOutputPath)
                            )
                        except Exception as e:
                            anx.debug(
                                "⚠️ Fail to write "
                            ...
                            anx.debug(traceback.format_exc())
                    anx.debug(
                        "🖼️ Camera img received: "
                        + str(self.__camImg.width)
                        + "x"
                        + str(self.__camImg.height)
                    )
        except Exception as e:
...
class OvaClientHttp(IRobot):
    def __init__(
        ...
        By default, the camera stream is disabled !
        You can enable it using enableCamera(True).
    ...
    def update(self, enableSleep=True):
        ...
        if dtTx > self.__dtTxToWait:
            self.__prevTx = now
            self._onUpdateRequests(
                self.__url + self.__robotControlRoute, self.__robotActuatorsRequest, now
            )
        if enableSleep:
    ...
    def setMotorSpeed(
        ...
        self.__robotActuatorsRequest.setMotorSpeed(
            leftPower, rightPower, durationInMsecs
        )
        ...
        try:
            ...
            if r.status_code != 200:
                ...
                try:
                    ...
                    anx.error(
                        "⚠️ Exception during loading json from " + url + " : " + str(e)
                    )
                    anx.error(traceback.format_exc())
        except Exception as e:
            ...
            if (
                self.__wasConnectedToRobot == True
                and self.isConnectedToRobot() == False
            ):
                self.__wasConnectedToRobot = False
    ...
    def _onUpdateRequests(self, url: str, request: RobotRequestBuilder, now=None):
        ...
        try:
            ...
            if len(content) > 0:
                ...
                dtPost = (datetime.now() - beforePost).total_seconds() * 1000
                if r.status_code != 200:
                    anx.warning(
                        "Return "
                        + str(r.status_code)
...
class OvaClientHttpV1(OvaClientHttp):
    ...
    def getBatteryVoltage(self) -> int:
        state = self.getRobotState()
        if (
            "sensors" not in state
            or "battery" not in state["sensors"]
            or "voltage" not in state["sensors"]["battery"]
        ):
            return 0
    ...
    def getFrontLuminosity(self) -> int:
        ...
        if (
            "sensors" not in state
            or "photoFront" not in state["sensors"]
            or "lum" not in state["sensors"]["photoFront"]
        ):
            return 0
    ...
    def getBackLuminosity(self) -> int:
        ...
        if (
            "sensors" not in state
            or "photoBack" not in state["sensors"]
            or "lum" not in state["sensors"]["photoBack"]
        ):
            return 0
    ...
    def setMotorAnimation(self, moves: list[tuple[int, int, int]]):
        if len(moves) > 1:
            anx.warning(
                "setMotorAnimation implemented for only 1 move in ova http client v1"
            )
...
class OvaClientHttpV2(OvaClientHttp):
    def __init__(
        ...
        ### Arguments
        * `url` - The http url to join ova on a LAN or WAN network, e.g. http://192.168.71.1
        * `verbosity` - The level of logs as an int. See Verbosity class for more info.
        * `imgOutputPath` - The path (either absolute or relative) where to save camera image on each update call
        * `welcomePrint` - True to print a nice message in the begining to welcome and guide you
        """
...
class OvaClientMqtt(IRobot):
    def __init__(
        ...
        * `welcomePrint` - True to print a nice message in the begining to welcome and guide you
        ...
        self.__cameraEnabled: bool = True
        ...
        try:
            if self.__isConnectedToBroker == False:
                anx.info(
                    ...
                    + str(self.__id)
                    + " to broker "
                    + self.__serverAddress
                    + ":"
                    + str(self.__serverPort)
                    + " ..."
                )
    ...
    def isConnectedToArena(self) -> bool:
        ...
        return (
            self.__isConnectedToBroker
            and dtRx < DefaultClientSettings.isConnectedTimeout
        )
